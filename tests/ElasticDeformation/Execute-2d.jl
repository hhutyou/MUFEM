...
const fix_x = union(nodes_left, nodes_right)
...
#----------- Material model -----------#
const mat = LinearElastic(3e8, 0.2)   # E = 3e8 Pa, ν = 0.2

# plane-strain elasticity tensor De for each element
De = Array{Float64, 3}(undef, 3, 3, nel)
let λ = mat.E * mat.ν / ((1.0 + mat.ν) * (1.0 - 2.0mat.ν)), μ = mat.E / (2.0 * (1.0 + mat.ν))

	D = [λ+2μ λ 0.0; λ λ+2μ 0.0; 0.0 0.0 μ]
	for iel ∈ 1:nel
		De[:, :, iel] = D
	end
end
...
const u_increment, u_increment_total = 0.1, 1.0
const traction = 0.0
